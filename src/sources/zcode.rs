...
//!
//! The store grows in place as sessions progress, so the parser replays the
//! whole database on every refresh (like the Jcode and Antigravity stores);
//! `prepare_file_task` pairs it with a WAL-aware file identity and
//! delete-first re-parses. Token usage comes from the `model_usage` table,
...
pub fn matches_path(path: &str) -> bool {
    path.ends_with("/.zcode/cli/db/db.sqlite") || path.ends_with("\\.zcode\\cli\\db\\db.sqlite")
}
...
pub(crate) fn parse_index_records(
    ...
    // Whole-database replay: `prepare_file_task` pairs this source with
    // delete-first re-parses, so `state.offset` is intentionally ignored.
    ...
    let mut sessions_statement = transaction
        .prepare(
            "SELECT id, parent_id, directory, time_created FROM session ORDER BY time_created, id",
        )
    ...
    let mut messages_statement = transaction
        .prepare(
            "SELECT m.id, m.sequence, m.time_created, m.data, \
             ...
             WHERE m.session_id = ?1 ORDER BY m.sequence, p.sequence",
        )
            ...
            match part_type {
                ...
                "tool" => {
                    ...
                    let output = part
                        ...
                        .and_then(value_to_string);
                    // Tool input is stored but not indexed as text, so it would
                    // be out of reach of search; carry it as `text`.
                    let text = input.clone().unwrap_or_default();
                    emit(Record {
    ...
    Ok(IndexParseOutput {
        ...
        // One store holds many sessions; there is no single file-level id.
        session_id: None,
        diagnostics,
        session_cwd,
    })
...
mod tests {
    ...
    fn records_carry_roles_tools_reasoning_and_hierarchy() {
        ...
        assert_eq!(tool.tool_output.as_deref(), Some("total 0"));
    ...
    fn usage_events_map_model_request_rows() {
        ...
        assert_eq!(main.project.as_deref(), Some("nipponhomes"));
