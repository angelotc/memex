...
impl StateLedger {
    ...
    pub fn is_processed(&self, source: &str, session_id: &str) -> bool {
        self.conn
            .query_row(
                "SELECT outcome FROM processed WHERE source = ? AND session_id = ?",
                params![source, session_id],
                |row| row.get::<_, String>(0),
            )
            .map(|outcome| outcome == "ok" || outcome == "skipped_trivial")
            .unwrap_or(false)
    }
    ...
    pub fn mark_processed(
        ...
        self.conn.execute(
            "INSERT INTO processed (source, session_id, processed_at, outcome, pattern_ids)
             VALUES (?, ?, ?, ?, ?)
...
mod tests {
    ...
    fn processed_roundtrip() {
        let (led, _tmp) = ledger();
        assert!(!led.is_processed("claude", "s1"));
        led.mark_processed("claude", "s1", "ok", &["pat_1".into()])
            .expect("mark");
        assert!(led.is_processed("claude", "s1"));
        // error outcome does not count as processed
        led.mark_processed("claude", "s2", "error", &[])
            .expect("mark");
        assert!(!led.is_processed("claude", "s2"));
    }
